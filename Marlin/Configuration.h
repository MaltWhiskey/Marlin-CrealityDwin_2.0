...
/*
   ...
   Mosquito may be set with either mounting option above and overrides temp sensor and max
   temp E3D Mounting assumes Groovemount Creality Mounting assumes bolt-on kit
*/
//#define HotendStock
//#define HotendE3D
...
// Enable this if you used a plug and play creality e3d or mosquito kit and kept the
// Creality thermistor
...
//#define SolidBedMounts //Removed a few LCD options to save some memory since not needed
// with solid mounts
...
//#define E3D_DUALFAN_MOUNT // Using HD Modular mount as above with 2 5015 blowers and
// sensor on the right #define E3D_PROBEMOUNT_LEFT // Default is probe mounted to the
// right for E3D. Set this to invert.
...
// Touchscreen options - only 32 bit boards have the open serial ports to use with
// graphics displays above
//#define ForceCRXDisplay
//#define Force10SProDisplay

//#define AddonFilSensor //Adds a filament runout sensor to the CR20 or Ender 4
//#define lerdgeFilSensor //Using lerdge filament sensor, which is opposite polarity to
// stock #define DualFilSensors //Using dual filament sensors on XMax and YMAX #define
// FilamentEncoder //Using filamet jam sensor such as the Bigtreetech Encoder wheel
...
/*
   ...
   Even just a probe and leveling with standard options is filling te entire program
   space!
*/
//#define OrigLA // Enable this for linear advance instead of mesh leveling on a melzi
// board #define MelziHostOnly // Enable this to turn off local SD support and instead
// prioritize options for Octoprint or USB
...
//#define CrealitySilentBoard // Creality board with TMC2208 Standalone drivers. Disables
// Linear Advance
...
//#define SKR13_ReverseSteppers // Some users reported directions backwards than others on
// SKR with various drivers. #define DualZ // Uses 5th driver on CRX or SKR boards as Z2
...
//#define POWER_LOSS_RECOVERY //Large and does not fit with any other features on Melzi,
// or UBL on Atmega

// Add filtering to endstops when long extensions are used. Makes homing more reliable but
// less accurate. #define CableExtensionNoiseFilter
...
/*
   ...
   UBL and Extreme are recommended with solid bed mounts as it becomes a one time
   commissioning. Standard is recommended in most other scenarios.
*/
//#define MeshFast
//#define MeshStd
//#define MeshFine
...
/*
   Disables SD Sort, Autotemp, Arc support, Linear Advance (Unless overridden with OrigLA
   above), Big edit fonts, and a few other little things Intended to resolve issues with
   certain newer boards experiencing motion errors under load. Enables Slim Menus
*/
...
//#define EnclosureTempSensor // Uses PT100 Probe hooked to A12, only partially
// implemented upstream #define EnclosureHeater //Planned to use A11 to control heater
// upstream, and repurpose the unused y max as the fan output. Not yet fully implemented
// upstream

//#define UnstableTemps // define if temps are unstable and you need a temporary
// workaround
...
#define BedDC
    ...
    ANY(ABL_EZABL, ABL_NCSW, ABL_BLTOUCH, ABL_TOUCH_MI) &&    \
    NONE(SKR13, SKR14, SKR14Turbo, SKRPRO11)
#define Z_STOP_PIN 19
#endif

#if ANY(MachineEnder2, MachineEnder3, MachineEnder5, MachineCR10, MachineMini) && \
    NONE(Melzi_To_SBoardUpgrade, SKR13, SKRPRO11)
...
#define SUICIDE_PIN \
  12  // Power up method is triggering the kill. Need to add consistent inversion upstream
...
#if NONE(MachineEnder4, MachineCR10SPro, MachineCRX, MachineCR10Max, \
         MachineEnder5Plus) ||                                       \
    ENABLED(GraphicLCD)
#undef SolidBedMounts
#endif

#if NONE(MachineCR10Orig, MachineEnder4, MachineCR10SPro, MachineCRX, MachineCR10Max, \
         MachineEnder5Plus) ||                                                        \
    ENABLED(GraphicLCD)
 ...
 * This allows the connection of wireless adapters (for instance) to non-default port
 * pins. Serial port -1 is the USB emulated serial port, if available. Note: The first
 * serial port (-1 or 0) will always be used by the Arduino bootloader.
 *
 * :[-1, 0, 1, 2, 3, 4, 5, 6, 7]
 */
...
#if ANY(SKR13, SKR14, SKR14Turbo, SKRPRO11) &&                               \
    (NONE(MachineCR10SPro, MachineCRX, MachineEnder5Plus, MachineCR10Max) || \
     (ENABLED(GraphicLCD) && NONE(Force10SProDisplay, ForceCRXDisplay)))
...
#define EXTRUDERS 1
#endif
// Generally expected filament diameter (1.75, 2.85, 3.0, ...). Used for Volumetric,
// Filament Width Sensor, etc.
...
//#define SWITCHING_NOZZLE_E1_SERVO_NR 1          // If two servos are used, the index of
// the second
...
#define PARKING_EXTRUDER_SOLENOIDS_INVERT  // If enabled, the solenoid is NOT magnetized
...
//#define PRIME_BEFORE_REMOVE                       // Prime the nozzle before release
// from the dock
...
// position when changing). The offset has to be X=0, Y=0 for the extruder 0 hotend
// (default extruder). For the other hotends it is their distance from the extruder 0
// hotend.
#if (ENABLED(Dual_ChimeraDualNozzle))
#define HOTEND_OFFSET_X \
  { 0.0, 18.00 }  // (in mm) for each extruder, offset of the hotend on the X axis
#define HOTEND_OFFSET_Y \
  { 0.0, 0.00 }  // (in mm) for each extruder, offset of the hotend on the Y axis
#endif
...
/**
 * --NORMAL IS 4.7kohm PULLUP!-- 1kohm pullup can be used on hotend sensor, using correct
 * resistor and table
 *
 ...
 * temp. !! 5 : 100K thermistor - ATC Semitec 104GT-2/104NT-4-R025H42G (Used in ParCan &
 * J-Head) (4.7k pullup) 501 : 100K Zonestar (Tronxy X3A) Thermistor 512 : 100k RPW-Ultra
 * hotend thermistor (4.7k pullup) 6 : 100k EPCOS - Not as accurate as table 1 (created
 * using a fluke thermocouple) (4.7k pullup) 7 : 100k Honeywell thermistor 135-104LAG-J01
 * (4.7k pullup) 71 : 100k Honeywell thermistor 135-104LAF-J01 (4.7k pullup) 8 : 100k 0603
 * SMD Vishay NTCS0603E3104FXT (4.7k pullup) 9 : 100k GE Sensing AL03006-58.2K-97-G1 (4.7k
 * pullup) 10 : 100k RS thermistor 198-961 (4.7k pullup) 11 : 100k beta 3950 1% thermistor
 * (4.7k pullup) 12 : 100k 0603 SMD Vishay NTCS0603E3104FXT (4.7k pullup) (calibrated for
 * Makibox hot bed) 13 : 100k Hisens 3950  1% up to 300°C for hotend "Simple ONE " &
 * "Hotend "All In ONE" 15 : 100k thermistor calibration for JGAurora A5 hotend 18 : ATC
 * Semitec 204GT-2 (4.7k pullup) Dagoma.Fr - MKS_Base_DKU001327 20 : Pt100 with circuit in
 * the Ultimainboard V2.x with 5v excitation (AVR) 21 : Pt100 with circuit in the
 * Ultimainboard V2.x with 3.3v excitation (STM32 \ LPC176x....) 201 : Pt100 with circuit
 * in Overlord, similar to Ultimainboard V2.x 60 : 100k Maker's Tool Works Kapton Bed
 * Thermistor beta=3950 61 : 100k Formbot / Vivedino 3950 350C thermistor 4.7k pullup 66
 * : 4.7M High Temperature thermistor from Dyze Design 67 : 450C thermistor from
 * SliceEngineering 70 : the 100K thermistor found in the bq Hephestos 2 75 : 100k Generic
 * Silicon Heat Pad with NTC 100K MGB18-104F39050L32 thermistor 99 : 100k thermistor with
 * a 10K pull-up resistor (found on some Wanhao i3 machines)
 *
 *       1k ohm pullup tables - This is atypical, and requires changing out the 4.7k
 * pullup for 1k. (but gives greater accuracy and more stable PID) 51 : 100k thermistor -
 * EPCOS (1k pullup) 52 : 200k thermistor - ATC Semitec 204GT-2 (1k pullup) 55 : 100k
 * thermistor - ATC Semitec 104GT-2 (Used in ParCan & J-Head) (1k pullup)
 *
...
#define TEMP_HYSTERESIS \
  7  // (°C) Temperature proximity considered "close enough" to the target
#else
...
#define PID_MAX \
  BANG_MAX  // Limits current to nozzle while PID is active (see PID_FUNCTIONAL_RANGE
            // below); 255=full current
#define PID_K1 0.95  // Smoothing factor within any PID loop
#if ENABLED(PIDTEMP)
#define PID_EDIT_MENU  // Add PID editing to the "Advanced Settings" menu. (~700 bytes of
                       // PROGMEM)
#define PID_AUTOTUNE_MENU  // Add PID auto-tuning to the "Advanced Settings" menu. (~250
                           // bytes of PROGMEM)
//#define PID_DEBUG             // Sends debug data to the serial port.
//#define PID_OPENLOOP 1        // Puts PID in open loop. M104/M140 sets the output power
// from 0 to PID_MAX #define SLOW_PWM_HEATERS      // PWM with very low frequency (roughly
// 0.125Hz=8s) and minimum state time of approximately 1s useful for heaters driven by a
// relay #define PID_PARAMS_PER_HOTEND // Uses separate PID parameters for each extruder
//(useful for mismatched extruders)
// Set/get with gcode: M301 E[extruder number, 0-2]
#define PID_FUNCTIONAL_RANGE \
  10  // If the temperature difference between the target temperature and the actual
      // temperature is more than PID_FUNCTIONAL_RANGE then the PID will be shut off and
 ...
 * means 7.689Hz, which is fine for driving a square wave into a resistive load and does
 * not significantly impact FET heating. This also works fine on a Fotek SSR-10DA Solid
 * State Relay into a 250W heater. If your configuration is significantly different than
 * this and you don't understand the issues involved, don't use bed PID until someone else
 ...
 * When set to any value below 255, enables a form of PWM to the bed that acts like a
 * divider so don't use it unless you are OK with PWM on your bed. (See the comment on
 * enabling PIDTEMPBED)
 */
...
// from FOPDT model - kp=.39 Tp=405 Tdead=66, Tc set to 79.2, aggressive factor of .15 (vs
// .1, 1, 10)
#if ENABLED(MachineCR2020)
 ...
 * Stepper Drivers
 *
 * These settings allow Marlin to tune stepper driver timing and enable advanced options
 * for stepper drivers that support them. You may also override timing options in
 * Configuration_adv.h.
 *
 ...
 * 'TB6600', 'TMC2100', 'TMC2130', 'TMC2130_STANDALONE', 'TMC2160', 'TMC2160_STANDALONE',
 * 'TMC2208', 'TMC2208_STANDALONE', 'TMC2209', 'TMC2209_STANDALONE', 'TMC26X',
 * 'TMC26X_STANDALONE', 'TMC2660', 'TMC2660_STANDALONE', 'TMC5130', 'TMC5130_STANDALONE',
 * 'TMC5160', 'TMC5160_STANDALONE']
 */

#if ANY(SKR13, SKR14, SKR14Turbo, SKRPRO11, MachineCR10SV2, CrealitySilentBoard, \
        MachineCR10SPro, MachineCR10SProV2, MachineCR10Max) &&                   \
    DISABLED(SKR_UART)
#if ENABLED(SKR_2209)
...
#define DEFAULT_TRAVEL_ACCELERATION \
  300  // X, Y, Z acceleration for travel (non printing) moves
#elif (ANY(MachineMini, MachineCR20, MachineEnder2, MachineEnder3, MachineEnder4, \
...
#define MAX_FEEDRATE_EDIT_VALUES \
  { 1000, 1000, 25, 150 }  // ...or, set your own edit limits
#endif
...
#define MAX_ACCEL_EDIT_VALUES \
  { 2000, 2000, 250, 500 }  // ...or, set your own edit limits
#endif
 ...
 *
 * This probe is deployed and activated by moving the X-axis to a magnet at the edge of
 * the bed. By default, the magnet is assumed to be on the left and activated by a home.
 * If the magnet is on the right, enable and set TOUCH_MI_DEPLOY_XPOS to the deploy
 * position.
 *
...
//#define TOUCH_MI_DEPLOY_XPOS (X_MAX_BED + 2)  // For a magnet on the right side of the
// bed #define TOUCH_MI_MANUAL_DEPLOY                // For manual deploy (LCD menu)
#endif
...
#define NOZZLE_TO_PROBE_OFFSET \
  { -44, -10, 0 }
    ...
    DISABLED(MicroswissDirectDrive)
#define NOZZLE_TO_PROBE_OFFSET \
  { -60, -10, 0 }
...
#define Z_PROBE_LOW_POINT \
  -3  // Farthest distance below the trigger-point to go before stopping
...
// when probing #define DELAY_BEFORE_PROBING 200  // (ms) To prevent vibrations from
...
//#define UNKNOWN_Z_NO_RAISE // Don't raise Z (lower the bed) if Z is "unknown." For beds
// that fall when Z is powered off.
...
#define Z_HOMING_HEIGHT \
  4  // (in mm) Minimal z height before homing (G28) for Z clearance above the bed,
     // clamps, ...
#endif
...
#elif ENABLED(MachineCR10SPro)
// BED is physically 310 wide, but zero is 2.5mm from the edges.
#define X_BED_SIZE 305
// BED is physically 320 deep, but movement starts at 2.5mm from both edges.
 ...
 */
#if (NONE(MachineCR10Orig, MachineCR20, MachineEnder4, MachineEnder5, MachineCRX, \
          Melzi_To_SBoardUpgrade) ||                                              \
     ANY(AddonFilSensor, lerdgeFilSensor, DualFilSensors))
...
#define NUM_RUNOUT_SENSORS \
  2  // Number of sensors, up to one per extruder. Define a FIL_RUNOUT#_PIN for each.
#endif
...
//===========================================================================
#define MESH_INSET 1
#if NONE(MachineCR10SPro, MachineCRX, MachineEnder5Plus, MachineCR10Max, MachineEnder4, \
         OrigLCD) ||                                                                    \
    ENABLED(GraphicLCD)
...
#define UBL_Z_RAISE_WHEN_OFF_MESH \
  0  // When the nozzle is off the mesh, this value is used
 ...
 */
    ...
    (DISABLED(MachineCRX) || ENABLED(GraphicLCD))
...
#define LEVEL_CORNERS_INSET_LFRB \
  { 22, 22, 22, 22 }              // (mm) Left, Front, Right, Back insets
...
// - Move the Z probe (or nozzle) to a defined XY point before Z Homing when homing all
// axes (G28).
...
//
#define HOST_KEEPALIVE_FEATURE  // Disable this if your host doesn't like keepalive
                                // messages
...
#define NOZZLE_PARK_XY_FEEDRATE \
  50  // X and Y axes feedrate in mm/s (also used for delta printers Z axis)
...
#define NOZZLE_CLEAN_START_POINT \
  { X_MAX_POS, 40, (Z_MIN_POS + 1) }
#define NOZZLE_CLEAN_END_POINT \
  { (X_MAX_POS - 10), 40, (Z_MIN_POS + 1) }
 ...
 * 'sk':'Slovak', 'tr':'Turkish', 'uk':'Ukrainian', 'vi':'Vietnamese', 'zh_CN':'Chinese
...
#elif NONE(MachineCR10SPro, MachineCRX, MachineEnder5Plus, MachineCR10Max, OrigLCD, \
           MachineCR10Orig) ||                                                      \
    ENABLED(GraphicLCD)
...
//
#if ANY(MachineCR10SPro, MachineCRX, MachineEnder5Plus, MachineCR10Max) && \
    (DISABLED(GraphicLCD) || ANY(Force10SProDisplay, ForceCRXDisplay))
...
#define NEOPIXEL_PIXELS \
  30  // Number of LEDs in the strip, larger of 2 strips if 2 neopixel strips are used
...
// (ms) Delay  before the next move will start, to give the servo time to reach its target
// angle. 300ms is a good value but you can try less delay. If the servo can't reach the
// requested position, increase it.
#define SERVO_DELAY \
  { 300 }
